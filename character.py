...
class Character:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.total_hp = (
            self.stats["hp"] * (1 + self.stats["hp_per"]) + self.stats["hp_flat"]
        )
        self.curr_hp = self.total_hp

        self.total_atk = (
            self.stats["atk"] * (1 + self.stats["atk_per"]) + self.stats["atk_flat"]
        )

        self.total_def = (
            self.stats["def"] * (1 + self.stats["def_per"] - self.stats["def_ignore"])
            + self.stats["def_flat"]
        )

        self.total_spd = (
            self.stats["spd"] * (1 + self.stats["spd_per"]) + self.stats["spd_flat"]
        )
